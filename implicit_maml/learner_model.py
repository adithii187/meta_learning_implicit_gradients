...
#     def compute_explicit_outer_grad(self, task, lam=None):

#         w_init = self.get_params().clone()

#         # Inner loop with graph!
#         for _ in range(self.inner_steps):
#             self.inner_opt.zero_grad()
#             loss = self.get_loss(task['x_train'], task['y_train'])
#             if lam is not None:
#                 loss += self.regularization_loss(w_init, lam)
#             loss.backward(create_graph=True)  # explicit graph
#             self.inner_opt.step()

#         # Outer gradient (second order!)
#         val_loss = self.get_loss(task['x_val'], task['y_val'])
#         val_grad = torch.autograd.grad(val_loss, self.model.parameters())

#         flat_grad = torch.cat([g.view(-1) for g in val_grad])

#         # Reset params
#         self.set_params(w_init)

#         return flat_grad

    
#     def compute_first_order_maml(self, task, lam=None):
#         """
#         Compute explicit gradient for meta-update by unrolling inner loop.
#         Args:
#             task: single few-shot task dictionary {'x_train','y_train','x_val','y_val'}
#             lam: optional regularization tensor
#         Returns:
#             outer_grad: flattened gradient vector for meta-update
#         """
#         # Save initial parameters
#         w_init = self.get_params().clone()
        
#         # Inner loop: unroll gradient steps
#         for _ in range(self.inner_steps):
#             self.inner_opt.zero_grad()
#             loss = self.get_loss(task['x_train'], task['y_train'])
#             if lam is not None:
...
#         # Compute gradient of validation loss w.r.t meta parameters
#         val_loss = self.get_loss(task['x_val'], task['y_val'])
#         val_grad = torch.autograd.grad(val_loss, self.model.parameters())
#         flat_grad = torch.cat([g.contiguous().view(-1) for g in val_grad])
...
# filename: implicit_maml/learner_model.py
...
# --- Version-agnostic import for functional_call ---
try:
    # PyTorch 1.12+
...
class Learner:
    ...
    def get_loss(self, x, y, return_numpy=False):
        ...
        loss = self.loss_function(yhat, y)
        if return_numpy:
            return utils.to_numpy(loss).ravel()[0]
        return loss

    def predict(self, x, return_numpy=False):
        x = utils.to_device(x, self.use_gpu)
        yhat = self.model.forward(x)
        if return_numpy:
            return utils.to_numpy(yhat)
        return yhat

    def learn_task(self, task, num_steps=10, w_0=None, lam=0.0):
        xt, yt = task['x_train'], task['y_train']
        train_loss = []
        for i in range(num_steps):
            self.inner_opt.zero_grad()
    ...
    def outer_step_with_grad(self, grad, flat_grad=False):
        if any(p.grad is None for p in self.model.parameters()):
            dummy_loss = 0.0
            for p in self.model.parameters():
                dummy_loss += torch.sum(p * 0.0)
            dummy_loss.backward()
        ...
        if flat_grad:
            ...
            grad = utils.to_device(grad, self.use_gpu)
            for p in self.model.parameters():
                p.grad.copy_(grad[offset:offset + p.nelement()].view(p.size()))
                offset += p.nelement()
        else:
            for i, p in enumerate(self.model.parameters()):
                p.grad = grad[i]
    ...
    def hessian_vector_product(self, task, vector, params=None):
        xt, yt = task['x_train'], task['y_train']
        if params is not None:
            self.set_params(params)
        tloss = self.get_loss(xt, yt)
    ...
    def compute_maml_grad(self, task):
        ...
        for _ in range(self.inner_steps):
            ...
            grads = torch.autograd.grad(loss, temp_params.values(), create_graph=True)
            temp_params = {n: p - self.inner_opt.param_groups[0]['lr'] * g for (n, p), g in zip(temp_params.items(), grads)}
    ...
    def compute_foml_grad(self, task):
        val_loss = self.get_loss(task['x_val'], task['y_val'])
        val_grad = torch.autograd.grad(val_loss, self.model.parameters())
        flat_grad = torch.cat([g.contiguous().view(-1) for g in val_grad])
        return flat_grad
    ...
    def compute_penalty_grad(self, task, penalty_lambda):
        """
        Computes the meta-gradient using a penalty-based method.
        The objective is L_val + lambda * ||grad(L_tr)||^2.
        """
        temp_params = {n: p for n, p in self.model.named_parameters()}
        
        # 1. Inner loop to get adapted parameters
        for _ in range(self.inner_steps):
            y_hat_tr = functional_call(self.model, temp_params, (task['x_train'],))
            train_loss = self.loss_function(y_hat_tr, task['y_train'])
            grads = torch.autograd.grad(train_loss, temp_params.values(), create_graph=True)
            temp_params = {n: p - self.inner_opt.param_groups[0]['lr'] * g for (n, p), g in zip(temp_params.items(), grads)}

        # 2. Calculate the two components of the total loss
        # Validation loss
        y_hat_val = functional_call(self.model, temp_params, (task['x_val'],))
        val_loss = self.loss_function(y_hat_val, task['y_val'])

        # Penalty term: squared norm of the inner-loop gradient at the adapted parameters
        final_train_y_hat = functional_call(self.model, temp_params, (task['x_train'],))
        final_train_loss = self.loss_function(final_train_y_hat, task['y_train'])
        grad_of_train_loss = torch.autograd.grad(final_train_loss, temp_params.values())
        
        penalty = sum(torch.sum(g**2) for g in grad_of_train_loss)

        # 3. Combine them into the total loss
        total_loss = val_loss + penalty_lambda * penalty
        
        # 4. The meta-gradient is the gradient of this total loss w.r.t original params
        outer_grad = torch.autograd.grad(total_loss, list(self.model.parameters()))
...
def make_conv_network(in_channels, out_dim, task='Omniglot'):
    assert task in ['Omniglot', 'MiniImageNet']
    
    model = nn.Sequential()
    num_filters = 64
    model.add_module('conv1', nn.Conv2d(in_channels=in_channels, out_channels=num_filters, kernel_size=3, stride=2, padding=1))
    model.add_module('BN1', nn.BatchNorm2d(num_filters, track_running_stats=False))
    model.add_module('relu1', nn.ReLU())
    model.add_module('conv2', nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=3, stride=2, padding=1))
    ...
    model.add_module('conv3', nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=3, stride=2, padding=1))
    model.add_module('BN3', nn.BatchNorm2d(num_filters, track_running_stats=False))
    model.add_module('relu3', nn.ReLU())
    model.add_module('conv4', nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=3, stride=2, padding=1))
